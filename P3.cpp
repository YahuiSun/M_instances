...
graph read_edges(graph input_graph, string staynerd_dimacs_file) {
	int v1;
	int v2;
	double weight;
...
#pragma endregion MST_P3_Yahui   2017年10月13日09:53:25
...
#pragma region 
void Growing_P3_3(graph initial_graph, graph& solu_graph, double& Growing_time) {
	...
	while (UV.size() > 0) { // while there is an unchecked vertex
		int target1 = UV[0]; // the root vertex
		// find a path candidate with at most 3 new vertices
		bool end = false;
		typedef boost::graph_traits<graph>::adjacency_iterator AdjacencyIterator;
		AdjacencyIterator ai1, a_end1;
		boost::tie(ai1, a_end1) = boost::adjacent_vertices(target1, initial_graph);
					...
					for (; ai2 != a_end2; ai2++) {
						...
						if (included[target3] == false) {
							...
							if (end == true) {
								...
								AdjacencyIterator ai3, a_end3;
								boost::tie(ai3, a_end3) = boost::adjacent_vertices(target3, initial_graph);
								for (; ai3 != a_end3; ai3++) {
									int target4 = *ai3;
									if (included[target4] == false && target4 != target2) { // it's not a circle
										double net_prize3 = get(boost::vertex_name_t(), initial_graph, target4) -
											get(boost::edge_weight_t(), initial_graph, boost::edge(target3, target4, initial_graph).first) + net_prize2;
										if (net_prize3 > 0) {
											end = true;
											// add the path candidate: target2, target3, target4
											boost::add_edge(target1, target2, get(boost::edge_weight_t(), initial_graph, boost::edge(target1, target2, initial_graph).first), solu_graph);
											boost::add_edge(target2, target3, get(boost::edge_weight_t(), initial_graph, boost::edge(target2, target3, initial_graph).first), solu_graph);
											boost::add_edge(target3, target4, get(boost::edge_weight_t(), initial_graph, boost::edge(target3, target4, initial_graph).first), solu_graph);
											included[target2] = true;
											included[target3] = true;
											included[target4] = true;
											if (in_degree(target2, solu_graph) < in_degree(target2, initial_graph)) {
												UV.insert(UV.end(), target2);
											}
											if (in_degree(target3, solu_graph) < in_degree(target3, initial_graph)) {
											...
											if (in_degree(target4, solu_graph) < in_degree(target4, initial_graph)) {
												UV.insert(UV.end(), target4);
											}
										}
										if (end == true) {
											break;
...
#pragma endregion Growing_P3_3


#pragma region 
void Growing_P3_1(graph initial_graph, graph& solu_graph, double& Growing_time) {
	...
	while (UV.size() > 0) { // while there is an unchecked vertex
		int target1 = UV[0]; // the root vertex
		// find a path candidate with at most 1 new vertex
		bool end = false;
		typedef boost::graph_traits<graph>::adjacency_iterator AdjacencyIterator;
		AdjacencyIterator ai1, a_end1;
		boost::tie(ai1, a_end1) = boost::adjacent_vertices(target1, initial_graph);
...
void P3(graph initial_graph, graph& solu_graph, double& P3_time) {
	...
	while (changed == true) {
		...
		if (round(new_cost*1e5)== round(old_cost*1e5)) {
			changed = false;
		}
	}
...
void P3_g1(graph initial_graph, graph& solu_graph, double& P3_time) {
	...
	while (changed == true) {
		...
		// TGA
		double Growing_time;
		Growing_P3_1(initial_graph, solu_graph, Growing_time);
		P3_time = P3_time + Growing_time;
		//cout << "TGA: " << net_cost(solu_graph) << endl;
		// MST
		double MST_time;
...
#pragma endregion P3_g1


#pragma region 

void P3_g3(graph initial_graph, graph& solu_graph, double& P3_time) {
	...
	while (changed == true) {
		...
		// TGA
		double Growing_time;
		Growing_P3_3(initial_graph, solu_graph, Growing_time);
		P3_time = P3_time + Growing_time;
		//cout << "TGA: " << net_cost(solu_graph) << endl;
		// MST
		double MST_time;
...
#pragma endregion P3_g3


#pragma region
void save_data_P3(string instance_name, graph result_graph, double net_cost, double running_time) {

	string save_name = "P3_" + instance_name; // save_name
	ofstream outputFile;
...
#pragma endregion save_data_P3


#pragma region 
void Post_process_DSN_FGW() {
	string instance = "D";
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_DSN_FGW.csv");
	outputFile << "P3_Result_DSN_FGW.csv" << endl;
	...
	for (int name = 1; name < 21; name++) {
		...
		if (name != 6 && name != 16) {
			// result_file
			if (name < 10) {
				initial_file = instance + "_0" + to_string(name) + "_a";
			}
			else if (name == 10) {
				initial_file = instance + "_" + to_string(name) + "_a";
			}
			else if (name < 20) {
				initial_file = instance + "_0" + to_string(name - 10) + "_b";
			}
			else if (name == 20) {
				initial_file = instance + "_" + to_string(name - 10) + "_b";
			}
			cout << initial_file << ":";
			string result_file = "Result_FGW1_" + initial_file;

			graph initial_graph = read_data_with_terminals(initial_file + ".stp");
			graph solu_graph = read_edges(initial_graph,result_file + ".stp");

			double P3_time = 0;
			double old_cost = net_cost(solu_graph);
			P3(initial_graph, solu_graph, P3_time);
			double new_cost = net_cost(solu_graph);
			save_data_P3(result_file, solu_graph, new_cost, P3_time); // save P3 data

			if (new_cost + 1e-5 < old_cost) {
				outputFile << initial_file << "," << old_cost << ",\\textbf{" << new_cost << "},"
					<< P3_time / 1000 << "s" << endl;
			}
			else {
				outputFile << initial_file << "," << old_cost << "," << new_cost << ","
					<< P3_time / 1000 << "s" << endl;
			}
		}
	}
...
#pragma endregion  Post_process_DSN_FGW


#pragma region 
void Post_process_PPI_FGW() {
	string instance = "PPI";
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_PPI_FGW.csv");
	outputFile << "P3_Result_PPI_FGW.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;
	...
	for (int name = 1; name < 11; name++) {
		...
		// instance_name
		initial_file = instance + "_" + to_string(name);
		cout << initial_file << ":";
		string result_file = "Result_FGW1_" + initial_file;
...
#pragma endregion  Post_process_PPI_FGW


#pragma region 
void Post_process_hand_FGW() {
	string instance;
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_hand_FGW.csv");
	outputFile << "P3_Result_hand_FGW.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;
	...
	for (int name = 1; name < 49; name++) {
		...
		if (name < 10) {
			instance = "handbd";
			initial_file = instance + to_string(0) + to_string(name);
		}
		else if (name < 15) {
			instance = "handbd";
			initial_file = instance + to_string(name);
		}
		else if (name < 24) {
			instance = "handbi";
			initial_file = instance + to_string(0) + to_string(name - 14);
		}
		else if (name < 29) {
			instance = "handbi";
			initial_file = instance + to_string(name - 14);
		}
		else if (name < 38) {
			instance = "handsd";
			initial_file = instance + to_string(0) + to_string(name - 28);
		}
		else if (name == 38) {
			instance = "handsd";
			initial_file = instance + to_string(name - 28);
		}
		else if (name < 48) {
			instance = "handsi";
			initial_file = instance + to_string(0) + to_string(name - 38);
		}
		else if (name == 48) {
			instance = "handsi";
			initial_file = instance + to_string(name - 38);
		}
		cout << initial_file << ":";
		string result_file = "Result_FGW1_" + initial_file;
...
#pragma endregion  Post_process_hand_FGW


#pragma region 
void Post_process_M_FGW() {
	string instance = "M";
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_M_FGW.csv");
	outputFile << "P3_Result_M_FGW.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;
	...
	for (int name = 1; name < 41; name++) {
		...
		if (name < 21) {
			initial_file = instance + to_string(name) + "A";
		}
		else {
			initial_file = instance + to_string(name - 20) + "B";
		}
		cout << initial_file << ":";
		string result_file = "Result_FGW1_" + initial_file;
		...
		double P3_time = 0;
		double old_cost = net_cost(solu_graph);
		P3_g1(initial_graph, solu_graph, P3_time);
		double new_cost = net_cost(solu_graph);
		save_data_P3(result_file, solu_graph, new_cost, P3_time); // save P3 data
...
#pragma endregion  Post_process_M_FGW


#pragma region 
void Post_process_DSN_MST_GPrA() {
	string instance = "D";
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_DSN_MST_GPrA.csv");
	outputFile << "P3_Result_DSN_MST_GPrA.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;
	...
	for (int name = 1; name < 21; name++) {
		...
		if (name != 6 && name != 16) {
			...
			cout << initial_file << ":";
			string result_file = "Result_MST_GPrA_" + initial_file;
...
#pragma endregion  Post_process_DSN_MST_GPrA


#pragma region 
void Post_process_PPI_MST_GPrA() {
	string instance = "PPI";
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_PPI_MST_GPrA.csv");
	outputFile << "P3_Result_PPI_MST_GPrA.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;
...
#pragma endregion  Post_process_PPI_MST_GPrA


#pragma region 
void Post_process_hand_MST_GPrA() {
	string instance;
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_hand_MST_GPrA.csv");
	outputFile << "P3_Result_hand_MST_GPrA.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;

	string initial_file;

	for (int name = 29; name < 49; name++) {
...
#pragma endregion  Post_process_hand_MST_GPrA


#pragma region 
void Post_process_M_MST_GPrA() {
	string instance = "M";
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("P3_Result_M_MST_GPrA.csv");
	outputFile << "P3_Result_M_MST_GPrA.csv" << endl;
	outputFile << "Instance,Old Cost,New Cost,Time" << endl;
...
#pragma endregion  Post_process_M_MST_GPrA


#pragma region 
void Post_process_hand_MST_GPrA_different_growing_n() {
	string instance;
	ofstream outputFile;
	outputFile.precision(4);
	outputFile.setf(ios::fixed);
	outputFile.setf(ios::showpoint);
	outputFile.open("Post_process_hand_MST_GPrA_different_growing_n.csv");
	outputFile << "Instance, App. ratio" << endl;
	...
	for (int name = 32; name < 33; name++) {
		...
		double P3_time = 0;
		double old_cost = net_cost(solu_graph);
		outputFile << initial_file << "," << old_cost / SMT_cost << "," << endl;
		cout << old_cost << endl;
		P3_g1(initial_graph, solu_graph, P3_time);
		double new_cost = net_cost(solu_graph);
		cout << new_cost << endl;
		//save_data_P3(result_file, solu_graph, new_cost, P3_time); // save P3 data
		outputFile << initial_file << "," << new_cost/SMT_cost << ","
			<< P3_time / 1000 << "s" << endl;

		//n=2
		solu_graph = read_edges(initial_graph, result_file + ".stp");
		P3(initial_graph, solu_graph, P3_time);
		new_cost = net_cost(solu_graph);
		cout << new_cost << endl;
		outputFile << initial_file << "," << new_cost / SMT_cost << ","
			<< P3_time / 1000 << "s" << endl;
		...
		//n=3
		solu_graph = read_edges(initial_graph, result_file + ".stp");
		P3_g3(initial_graph, solu_graph, P3_time);
		new_cost = net_cost(solu_graph);
		cout << new_cost << endl;
		outputFile << initial_file << "," << new_cost / SMT_cost << ","
			<< P3_time / 1000 << "s" << endl;
...
#pragma endregion  Post_process_hand_MST_GPrA_different_growing_n
